...
$sql = "SELECT id_ads, title, ads_desc, imageads, curr, price FROM nukec30_ads_ads where active = '1' and validuntil > '".$mktimeUnixblock."' ORDER BY id_ads DESC LIMIT 0,3";
$result = $db->sql_query($sql);
if ($db->sql_numrows($result)) {
$content = '<a href="/nukecbackend.php"><img src="/images/blocks/nukecrss.gif" alt="RSS" border="0" /></a><br /><br />';
 $content .= '<center><table width="100%" cellspacing="3" cellpadding="2">';
   
    while (list($id_ads, $title, $ads_desc, $imageads, $currencycodeB, $valuepriceB) = $db->sql_fetchrow($result)) {
    $content .= '<tr><td align="center" rowspan="3" bgcolor="'.$bgcolor2.'">'; 
        $title = stripslashes($title);
$title =wordwrap($title, 30, "<br />\n");
        $sid = intval($id_ads);
       $ads_desc = wordwrap($ads_desc, 30, "<br />"); 
      $ads_desc = strip_tags(substr($ads_desc, 0, 100));

         $content .= '<a href="modules.php?name=NukeC&amp;op=ViewDetail&amp;id_ads='.$sid.'"><strong>'.$title.'</strong></a><br />';   
        $content .= ''.$ads_desc.'';
if ($imageads == "") {
$content .= '<br /><img alt="no image" src="modules/NukeC/imagecatg/noimage.gif" /><br />';
 ...
 if ($imageads != "") {
			...
			$content .= '<br /><img alt="'.$title.'" src="modules/NukeC/imageads/'.$imageads_thumb.'" width="42" height="42" /><br />';
			}
		} 
