...
/*******************************************************************************/
if (preg_match("/block-NukeC30Most.php/", $_SERVER['SCRIPT_NAME'])) {
...
$sqllatest = "select id_ads,title, ads_desc, imageads, submitter, curr, price from ".$nukecprefix."_ads_ads where active = '1' and validuntil > '".$mktimeUnixblock."' order by hits DESC limit 0,".$NumberAdsBlock;
...
if ($db->sql_numrows($resultlatest) > 0) {
	...
	while (list ($id_ads, $title, $ads_descB, $imageads, $submitter, $currencycodeB, $valuepriceB) =$db->sql_fetchrow($resultlatest)) {
		...
		$its_url = "<a href=\"modules.php?name=NukeC30&op=ViewDetail&id_ads=$id_ads\">details &raquo;&raquo; </a>";
		$ads_descBroke = strip_tags(BreakQuoteatMost ($ads_descB, $Chars2Break, $its_url));
		$content .= "$ads_descBroke<br />";
		$content .= "</td>";
		$content .= "</tr>";
		$content .= "</table>";
	}
	$content .= "<tr><td align=\"right\"><a href=\"modules.php?name=NukeC30\">More ads like this &raquo;&raquo;</a></td></tr>";
	$content .= "</table><br />";
	$db->sql_freeresult($resultlatest);
} else {
	$content = "No Ads submitted yet";
}
function BreakQuoteatMost ($string_to_break, $CharsLimit, $its_url) {
