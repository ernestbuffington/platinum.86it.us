...
/*******************************************************************************/
// Note: You can't use center block and side block at the same time.
//       You can only use either one of them.
if (preg_match("/block-NukeC30Center.php/", $_SERVER['SCRIPT_NAME'])) {
...
$sqllatest = "select id_ads,title, ads_desc, imageads, submitter, curr, price from ".$nukecprefix."_ads_ads where active = '1' and validuntil > '".$mktimeUnixblock."' order by rand() DESC limit 0,".$NumberAdsBlock;
...
if ($db->sql_numrows($resultlatest) > 0) {
	...
	while (list ($id_ads, $title, $ads_descB, $imageads, $submitter, $currencycodeB, $valuepriceB) =$db->sql_fetchrow($resultlatest)) {
		...
		$content .= "<td valign=\"top\" width=\"180\">";
		$content .= "<table width=\"100%\" cellspacing=\"2\" cellpadding=\"2\" border=\"0\">";
		...
		$content .= "$ads_descBroke</td></tr></table>";
	}
