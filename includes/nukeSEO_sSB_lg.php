...
function getBookmarkHTML3($mynukeurl, $mynuketitle, $separator = "&nbsp;", $imgsize = "large")
{
###############################################################################
...
###############################################################################
	...
	foreach ($bookmarks as $sitename => $sitedetails)
	{
		...
		$bookmarkHTML3 .= "<a href=\"$siteurl\" title=\"$imgalt\" target=\"_blank\" class=\"mlg_donkeytopic\">";
		$siteimg = 'modules/News/css/images/transparent.gif';
		if ($imgsize == "text") {
			$bookmarkHTML3 .= "$sitename";
		} else {
			# XHTML fix courtesy of Guardian - http://code-authors.com
			$bookmarkHTML3 .= "<img alt=\"\" border=\"0\" class=\"mlg_sbicon_med $imgclass\" src=\"$siteimg\" style=\"opacity:0.4;filter:alpha(opacity=40)\" onmouseover=\"this.style.opacity=1;this.filters.alpha.opacity=100\" onmouseout=\"this.style.opacity=0.4;this.filters.alpha.opacity=40\" />";
		}
		$bookmarkHTML3 .= "</a>";
		$numkey = $numkey + 1;
		//if ($numkey < $numBookmarks) $bookmarkHTML .= "$separator";
	}
	return $bookmarkHTML3;
}
?>
