...
//
        ...
        $text = preg_replace_callback("(\[ul\](.*?)\[/ul\])is", function($m) { return '<ul>'.$m[1].'</ul>'; }, $text);
        $text = preg_replace_callback("(\[li\](.*?)\[/li\])is", function($m) { return '<li>'.$m[1].'</li>'; }, $text); 
		...
		$text = preg_replace_callback("(\[url\]((www|ftp)\.[^ \[\"\n\r\t<]*?)\[/url\])is", function($m) { return '<a href="http://'.$m[1].'" target="_blank" title="'.$m[1].'">'.shrink_url($m[1]).'</a>'; }, $text);
