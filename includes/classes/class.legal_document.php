...
class Legal_Document
{
	...
	function dbGetDocument($doc='', $lang='')
	{
		...
		/*
		 * Go get the document data.  First assume we have the most likely scenario where the document exists as well
		 * as the translation text for the document Title and Document Text.  Then, if this fails, at least get the
		 * document information and not text.
		 */
		$sql = 'SELECT a.`did` AS doc_id, `doc_name`, `doc_status`, c.`doc_text` AS doc_text, '
			...
			. LGL_MID_TYPE;
		if ($this->checkActive) {
			$sql .= ' AND `doc_status` = \'1\'';
		}
		$sql .= ' AND b.`language` = \'' . addslashes($this->doc['lang']) . '\''
		 ...
		 * the root document information and no text.
		 */
		if ($result = $db->sql_query($sql)) {
			$dbNumRows = $db->sql_numrows($result);
			if ($dbNumRows > 1) return false;  // Had either no row or too many (i.e., more than 1)
			if ($dbNumRows == 1) {
		...
		if (!$result = $db->sql_query($sql)) return false; // Document does not exist so definitly something is up
	...
	function html()
	{
		...
		$docText = preg_replace('#\[sitename\]#i', $sitename, $docText);
		$docText = preg_replace('#\[country\]#i', $this->docCountry, $docText);
		$docText = preg_replace('#\[date\]#i', $dateS, $docText);
