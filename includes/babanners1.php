...
if (stristr($_SERVER['PHP_SELF'], "banners1.php") || stristr($_SERVER['SCRIPT_NAME'], "banners1.php")) {
...
$testdate = date ("Y-m-d");
$bresult = $db->sql_query("SELECT bid FROM ".$prefix."_nsnba_banners WHERE pid='1' AND active='1'");
...
if ($numrows>0) {
  $bidinfo = $db->sql_fetchrow($db->sql_query("SELECT * FROM ".$prefix."_nsnba_banners WHERE pid='1' AND active='1' AND bid='$bannum'"));
  ...
  if ($bidinfo['flash']==1 AND $bidinfo['code']==0) {
    ...
    $babanners1  = "<a href='clicksba.php?bid=$bannum' target='_blank'><img src='".$bidinfo['imageurl']."' border='0' alt='".$bidinfo['alttext']."' title='".$bidinfo['alttext']."' height='".$bidinfo['height']."' width='".$bidinfo['width']."'></a>";
  }
} else {
  $babanners1 = "<a href='modules.php?name=$modname'><img src='modules/$modname/images/ban-banner-1.png' border='0' alt='$sitename' title='$sitename'></a>";
}
$bresult3 = $db->sql_query("SELECT bid, datestr FROM ".$prefix."_nsnba_banners WHERE pid='1' AND active='-1'");
