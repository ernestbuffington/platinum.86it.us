...
function getBookmarkHTML2($mynukeurl, $mynuketitle, $separator = "&nbsp;", $imgsize = "medum")
{
###############################################################################
...
###############################################################################
	...
	foreach ($bookmarks as $sitename => $sitedetails)
	{
		...
		$bookmarkHTML2 .= "<a href=\"$siteurl\" title=\"$imgalt\" target=\"_blank\" class=\"m_donkeytopic\">";
		$siteimg = 'modules/News/css/images/transparent.gif';
		if ($imgsize == "text") {
			$bookmarkHTML2 .= "$sitename";
		} else {
			# XHTML fix courtesy of Guardian - http://code-authors.com
			$bookmarkHTML2 .= "<img alt=\"\" border=\"0\" class=\"m_sbicon_med $imgclass\" src=\"$siteimg\" style=\"opacity:0.4;filter:alpha(opacity=40)\" onmouseover=\"this.style.opacity=1;this.filters.alpha.opacity=100\" onmouseout=\"this.style.opacity=0.4;this.filters.alpha.opacity=40\" />";
		}
		$bookmarkHTML2 .= "</a>";
		$numkey = $numkey + 1;
