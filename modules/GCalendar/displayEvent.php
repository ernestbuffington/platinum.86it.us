...
function gcalComputeEventNotes($startDate, $repeat, $interval, $noEndDate, $endDate, $weeklyDays, $monthlyByDay, 
   $dateFormat)
{
   $notes = '';
   if ($repeat != 'none')
...
function gcalComputeEventRsvp($event, $backTo, &$rsvpCount)
{
   ...
   $sql = 'SELECT r.user_id, u.username FROM ' . $prefix . GCAL_RSVP_TABLE . ' as r, ' .
      $prefix . '_users as u WHERE r.event_id = \'' . $event . '\' AND r.user_id = u.user_id ' .
      'ORDER BY u.username';
   $result = $db->sql_query($sql);
   $ids = array();
   $rsvpCount = 0;
