 ...
 * Refer to platinumnuke.com for detailed information on PHP-Platinum Nuke Pro
 ***************************************************************************/
/***************************************************************************
...
// START ERROR -pertains to 'Personal User' group?
...
/*****************************************************/
                $showquickreply = ( isset($_POST['showquickreply']) ) ? ( ($_POST['showquickreply']) ? TRUE : 0 ) : $board_config['allow_quickreply'];
/*****************************************************/
...
/*****************************************************/
		...
		$user_flag = ( !empty($_POST['user_flag']) && $_POST['user_flag'] != 'blank.gif' ) ? htmlspecialchars($_POST['user_flag']) : '' ;	// Country/Location Flags
...
/*****************************************************/
                $kicker_ban = ( !empty($_POST['kicker_ban']) ) ? intval( $_POST['kicker_ban'] ) : 0;
/*****************************************************/
/* Forum - Thread Kicker v.1.0.2                 END */
/*****************************************************/ 
        $user_rank = ( !empty($_POST['user_rank']) ) ? intval( $_POST['user_rank'] ) : 0;
/*****[BEGIN]******************************************/
/* Multiple Ranks And Staff View      PNPv1           */
/******************************************************/
...
/*****************************************************/
                        $sql = "UPDATE " . USERS_TABLE . "
                                SET " . $username_sql . $passwd_sql . "user_email = '" . str_replace("\'", "''", $email) . "', user_icq = '" . str_replace("\'", "''", $icq) . "', user_website = '" . str_replace("\'", "''", $website) . "', user_occ = '" . str_replace("\'", "''", $occupation) . "', user_from = '" . str_replace("\'", "''", $location) . "', user_from_flag = '" . str_replace("\'", "''", $user_flag) . "', user_interests = '" . str_replace("\'", "''", $interests) . "', user_birthday='$birthday', user_next_birthday_greeting=$next_birthday_greeting, user_sig = '" . str_replace("\'", "''", $signature) . "', user_viewemail = $viewemail, user_aim = '" . str_replace("\'", "''", $aim) . "', user_yim = '" . str_replace("\'", "''", $yim) . "', user_msnm = '" . str_replace("\'", "''", $msn) . "', user_attachsig = $attachsig, user_sig_bbcode_uid = '$signature_bbcode_uid', user_allowsmile = $allowsmilies, user_quickreply = $showquickreply, user_allowhtml = $allowhtml, user_allowavatar = $user_allowavatar, user_allowbbcode = $allowbbcode, user_allow_viewonline = $allowviewonline, user_notify = $notifyreply, user_allow_pm = $user_allowpm, kick_ban = $kicker_ban, user_notify_pm = $notifypm, user_popup_pm = $popuppm, user_lang = '" . str_replace("\'", "''", $user_lang) . "', user_style = $user_style, user_timezone = $user_timezone, user_dateformat = '" . str_replace("\'", "''", $user_dateformat) . "', user_active = $user_status, user_posts = $user_posts, user_rank = '$user_rank', user_rank2 = '$user_rank2', user_rank3 = '$user_rank3', user_rank4 = '$user_rank4', user_rank5 = '$user_rank5', user_gender = '$gender'" . $avatar_sql . "
                                WHERE user_id = $user_id";
/*****************************************************/
/* Forum - Thread Kicker v.1.0.2                 END */
/* Forum - Edit User Post Count v.1.0.1          END */
