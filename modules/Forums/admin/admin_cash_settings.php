...
if ( !empty($setmodules) )
{
	...
	$phpbb['Cash/Shop Mod']['Cash_Settings'] = "$file";
	return;
}
...
$template->assign_vars(array(
	...
	"L_CASH_INCLUDE_QUOTES" => $lang['Cash_include_once_quotes'],
...
while ( $c_cur = &$cash->currency_next($cm_i) )
{
	...
	$include_once_quotes_yes = ( $c_cur->mask(CURRENCY_INCLUDEQUOTES) ) ? "checked=\"checked\"" : "";
	$include_once_quotes_no = ( !$c_cur->mask(CURRENCY_INCLUDEQUOTES) ) ? "checked=\"checked\"" : "";
	...
	$template->assign_block_vars("cashrow",array(	"CASH_INDEX" => $c_cur->id(),
													...
													"INCLUDE_QUOTES_YES" => $include_once_quotes_yes,
													"INCLUDE_QUOTES_NO" => $include_once_quotes_no,
