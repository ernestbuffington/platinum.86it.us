...
if (!empty($mode))
{
	if ($mode == 'edit' || $mode == 'add')
	{
		...
		*/
		...
		if ($mode == 'edit')
		{
			...
			$sql = 'SELECT * FROM ' . BBATTRIBUTES . ' WHERE attribute_id = ' . intval($attribute_id);
		...
		{
			$sql = 'SELECT attribute_id, attribute_order FROM ' . BBATTRIBUTES . ' ORDER BY attribute_order DESC LIMIT 1';
			if (!$result = $db->sql_query($sql))
			{
	...
	else if ($mode == 'save')
	{
		...
		*/
		...
		$administrator		= $_POST['attribute_administrator'] ? 1 : 0;
		$moderator				= $_POST['attribute_moderator'] ? 1 : 0;
		$author						= $_POST['attribute_author'] ? 1 : 0;
		$date							= request_var('attribute_date_format', TYPE_NO_HTML);
		$position					= empty($_POST['attribute_position']) ? 0 : 1;
		$color						= request_var('attribute_color', TYPE_NO_HTML);
		$tmp_order				= request_var('attribute_tmp_order', TYPE_INT);
		$order						= isset($_POST['attribute_order']) ? (intval($_POST['attribute_order']) + 1) : $tmp_order;
		if ( empty($attribute) )
		{
		...
		if ( $attribute_id )
		{
			$sql = 'UPDATE ' . BBATTRIBUTES . ' SET attribute_type = \'' . str_replace("\'", "''", $type) . '\', attribute = \'' . str_replace("\'", "''", $attribute) . '\', attribute_image= \'' . str_replace("\'", "''", $attribute_image) . '\', attribute_color = \'' . str_replace("\'", "''", $color) . '\', attribute_date_format = \'' . str_replace("\'", "''", $date) . '\', attribute_position = ' . $position . ', attribute_administrator = ' . $administrator . ', attribute_moderator = ' . $moderator . ', attribute_author = ' . $author . ', attribute_order = ' . $order . ' WHERE attribute_id = ' . intval($attribute_id);
		...
		else
		{
			...
			$sql = 'INSERT INTO ' . BBATTRIBUTES . ' (attribute_type, attribute, attribute_image, attribute_color, attribute_administrator, attribute_moderator, attribute_author, attribute_date_format, attribute_position, attribute_order)
		...
		if ( $order != $tmp_order )
		{
			$sql = 'SELECT attribute_id FROM ' . BBATTRIBUTES . ' ORDER BY attribute_order';
			if (!$result = $db->sql_query($sql))
			{
			...
			{
				$inc += 10;
				$sql = 'UPDATE ' . BBATTRIBUTES . ' SET attribute_order = ' . $inc . ' WHERE attribute_id = ' . intval($row['attribute_id']);
				if (!$db->sql_query($sql))
				{
	...
	else if ($mode == 'delete')
	{
		...
		*/
		...
		{
			$sql = 'DELETE FROM ' . BBATTRIBUTES . ' WHERE attribute_id = ' . intval($attribute_id);
			if (!$result = $db->sql_query($sql))
			{
	...
	else if ( $mode == 'moveup' || $mode == 'movedw' )
	{
		...
		$sql = 'UPDATE ' . BBATTRIBUTES . ' SET attribute_order = attribute_order + ' . $inc . ' WHERE attribute_id = ' . intval($attribute_id);
		if ( !$db->sql_query($sql) )
		{
			message_die(GENERAL_ERROR, $lang['Attr_Error_Message_05'], '', __LINE__, __FILE__, $sql);
		}
...
else
{
	...
	*/
	...
	$sql = 'SELECT * FROM ' . BBATTRIBUTES . ' ORDER BY attribute_order ASC';
	if (!$result = $db->sql_query($sql))
	{
