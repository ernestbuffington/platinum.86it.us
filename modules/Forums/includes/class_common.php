...
function _butt($var)
{
	...
	if ( isset($_POST[$var . '_x']) && isset($_POST[$var . '_y']) )
	{
		$_POST[$var] = true;
	}
...
function _cvt_pic_butts()
{
	...
	if ( !empty($_POST) )
	{
		...
		foreach ( $_POST as $key => $val )
		{
			...
			if ( ($key == $var . '_x') && isset($_POST[$var . '_y']) )
			{
				$added[$var] = true;
			}
		}
...
{
	$cleaned_chars = array("\r\n" => "\n", "\r" => "\n", "\xFF" => ' ');
	$string = trim(htmlspecialchars(str_replace(array_keys($cleaned_chars), array_values($cleaned_chars), $string)));
...
function request_var($var, $type, $dft='', $list='')
{
	...
	if ( !empty($var) )
	{
		...
		{
			$res = isset($_POST[$var]) ? $_POST[$var] : urldecode($_GET[$var]);
			if ( isset($_POST[$var]) && is_array($res) )
			{
