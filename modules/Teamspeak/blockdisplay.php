...
//
//
// Modified by Fusion for Art of Gaming and Evo Clans
//  - www.artofgaming.co.uk   &   www.evolutionclans.com
...
class teamspeakDisplayClass {
	...
	function displayTeamspeakEx($settings) {
		...
		$content .="<div id=\"teamspeakdisplay\">\n";
		if ($serverInfo["queryerror"] != 0) {
			...
			$content .="<table><tr><td>";
			$content .="<img src=\"images/teamspeakdisplay/teamspeak_offline.png\" alt=\"\" title=\"" . $popupInfo . "\">";
			$content .="</td><td class=\"teamspeakserver\" title=\"" . $popupInfo . "\">";
			...
			$content .="function stringOk_" . $jsTeamspeakId . "(string, forbiddenChars) {\n";
			$content .="	for(var i = 0; i < string.length; i++) {\n";
			$content .="		if (forbiddenChars.indexOf(string.charAt(i)) > -1) {\n";
			$content .="			return false;\n";
			$content .="		}\n";
			$content .="	}\n";
			$content .="	return true;\n";
			$content .="}\n";
			$content .="function enterServer_" . $jsTeamspeakId . "() {\n";
			$content .="	enterSubChannel_" . $jsTeamspeakId . "(null, false, null);\n";
			$content .="}\n";
			$content .="function enterChannel_" . $jsTeamspeakId . "(channelName, channelPassworded) {\n";
			$content .="	enterSubChannel_" . $jsTeamspeakId . "(channelName, channelPassworded, null);\n";
			$content .="}\n";
			$content .="function enterSubChannel_" . $jsTeamspeakId . "(channelName, channelPassworded, subChannelName) {\n";
			$content .="	var serveraddress = 'teamspeak://" . $settings["serveraddress"] . ":" . $settings["serverudpport"] . "';\n";
			$content .="	var nickname=window.prompt('Enter your nickname', '');\n";
			$content .="	if (nickname == null) {\n";
			$content .="		return;\n";
			$content .="	} else if (! stringOk_" . $jsTeamspeakId . "(nickname, '" . str_replace("'", "\\'", $settings["forbiddennicknamechars"]) . "')) {\n";
			$content .="		window.alert('Could not enter the teamspeak server because the nickname you entered contains one or more of these forbidden characters: " . str_replace("'", "\\'", $settings["forbiddennicknamechars"]) . "');\n";
			$content .="		return;\n";
			$content .="	} else if (nickname == \"\") {\n";
			$content .="		window.alert('Could not enter the teamspeak server because you did not enter your nickname');\n";
			$content .="		return;\n";
			$content .="	}\n";
			$content .="	serveraddress = serveraddress + \"/nickname=\" + escape(nickname);\n";
			if ($serverInfo["serverinfo"]["server_password"] == "1") {
				$content .="	var password=window.prompt('Enter the teamspeak server password for " . $serverInfo["serverinfo"]["server_name"] . "', '');\n";
				$content .="	if (password == null) {\n";
				$content .="		return;\n";
				$content .="	} else if (password == \"\") {\n";
				$content .="		window.alert('Could not enter the teamspeak server because you did not enter a server password');\n";
				$content .="		return;\n";
				$content .="	}\n";
				$content .="	serveraddress = serveraddress + \"?password=\" + escape(password);\n";
			}
			$content .="	if (channelName != null) { serveraddress = serveraddress + \"?channel=\" + escape(channelName); }\n";
			$content .="	if (channelPassworded) {\n";
			$content .="		var channelpassword=window.prompt('Enter the channel password for channel ' + channelName, '');\n";
			$content .="		if (channelpassword == null) {\n";
			$content .="			return;\n";
			$content .="		} else if (channelpassword == \"\") {\n";
			$content .="			window.alert('Could not enter the teamspeak server because you did not enter a channel password');\n";
			$content .="			return;\n";
			$content .="		}\n";
			$content .="		serveraddress = serveraddress + \"?channelpassword=\" + escape(channelpassword);\n";
			$content .="	}\n";
			$content .="	if (subChannelName != null) { serveraddress = serveraddress + \"?subchannel=\" + escape(subChannelName); }\n";
			$content .="	window.location=serveraddress;\n";
			$content .="}\n";
			$content .="//--></script>\n";
			...
			$content .="<table><tr><td>";
			$content .="<img src=\"images/teamspeakdisplay/server.gif\" alt=\"\" title=\"" . $popupInfo . "\">";
			$content .="</td><td class=\"teamspeakserver\" title=\"" . $popupInfo . "\">";
			$content .="<a class=\"teamspeakserver\" href=\"javascript:enterServer_" . $jsTeamspeakId . "();\">";
			$content .=str_replace(" ", "&nbsp;", htmlspecialchars($serverInfo["serverinfo"]["server_name"]));
			$content .="</a>";
			$content .="</td></tr></table>\n";
			...
			foreach($serverInfo["channellist"] as $channelInfo) { if ($channelInfo["parent"] == -1) {
				...
				$content .="<table><tr><td>";
				$content .="<img src=\"images/teamspeakdisplay/treeimage" . ((($counter + 1) == $currentchannels) ? "3" : "2") . ".png\" alt=\"\">";
				$content .="<img src=\"images/teamspeakdisplay/channel.gif\" alt=\"\" title=\"" . $popupInfo . "\">";
				$content .="</td><td class=\"teamspeakchannel\" title=\"" . $popupInfo . "\">";
				$content .="<a class=\"teamspeakchannel\" href=\"javascript:enterChannel_" . $jsTeamspeakId . "('" . str_replace("'", "\'", $channelInfo["channelname"]) . "', " . (($channelInfo["password"]) == "1" ? "true" : "false") . ");\">";
				$content .=str_replace(" ", "&nbsp;", htmlspecialchars($channelInfo["displayname"]));
				$content .="</a>";
				$content .="</td></tr></table>\n";
				}
				}else{
				...
				foreach($serverInfo["playerlist"] as $playerInfo) {
					...
					if ($playerInfo["channelid"] == $channelInfo["channelid"]) {
						...
						// Display player:
						$content .="<table><tr><td>";
						$content .="<img src=\"images/teamspeakdisplay/treeimage" . ((($counter + 1) == $currentchannels) ? "4" : "1") . ".png\" alt=\"\">";
						$content .="<img src=\"images/teamspeakdisplay/treeimage" . ((($counter_playerandsubchannels + 1) == $currentplayersandsubchannels) ? "3" : "2") . ".png\" alt=\"\">";
						if ($channelInfo[channelname] == $settings["awaychannel"]) {
						$content .="<img src=\"images/teamspeakdisplay/player_away.png\" alt=\"" . $playerInfo["displayimage"] . "\" title=\"" . $popupInfo . "\">";
						}else{
						$content .="<img src=\"images/teamspeakdisplay/player_" . $playerInfo["displayimage"] . ".png\" alt=\"" . $playerInfo["displayimage"] . "\" title=\"" . $popupInfo . "\">";}
						$content .="</td><td class=\"teamspeakplayer\" title=\"" . $popupInfo . "\">";
						$content .=str_replace(" ", "&nbsp;", htmlspecialchars($playerInfo["displayname"]));
						$content .="</td></tr></table>\n";
				...
				foreach($serverInfo["channellist"] as $subchannelInfo) { if ($subchannelInfo["parent"] == $channelInfo["channelid"]) { 
					...
					$content .="<img src=\"images/teamspeakdisplay/treeimage" . ((($counter_playerandsubchannels + 1) == $currentplayersandsubchannels) ? "3" : "2") . ".png\" alt=\"\">";
					$content .="<img src=\"images/teamspeakdisplay/channel.gif\" alt=\"\" title=\"" . $popupInfo . "\">";
					$content .="</td><td class=\"teamspeaksubchannel\" title=\"" . $popupInfo . "\">";
					$content .="<a class=\"teamspeaksubchannel\" href=\"javascript:enterSubChannel_" . $jsTeamspeakId . "('" . str_replace("'", "\'", $channelInfo["channelname"]) . "', " . (($channelInfo["password"]) == "1" ? "true" : "false") . ", '" . str_replace("'", "\'", $subchannelInfo["channelname"]) . "');\">";
					$content .=str_replace(" ", "&nbsp;", htmlspecialchars($subchannelInfo["displayname"]));
					$content .="</a>";
					$content .="</td></tr></table>\n";
					...
					foreach($serverInfo["playerlist"] as $playerInfo) {
						...
						if ($playerInfo["channelid"] == $subchannelInfo["channelid"]) {
							...
							$content .="<table><tr><td>";
							$content .="<img src=\"images/teamspeakdisplay/treeimage" . ((($counter + 1) == $currentchannels) ? "4" : "1") . ".png\" alt=\"\">";
							$content .="<img src=\"images/teamspeakdisplay/treeimage" . ((($counter_playerandsubchannels + 1) == $currentplayersandsubchannels) ? "4" : "1") . ".png\" alt=\"\">";
							$content .="<img src=\"images/teamspeakdisplay/treeimage" . ((($counter_player + 1) == $currentplayers) ? "3" : "2") . ".png\" alt=\"\">";
							$content .="<img src=\"images/teamspeakdisplay/player_" . $playerInfo["displayimage"] . ".png\" alt=\"" . $playerInfo["displayimage"] . "\" title=\"" . $popupInfo . "\">";
							$content .="</td><td class=\"teamspeakplayer\" title=\"" . $popupInfo . "\">";
							$content .=str_replace(" ", "&nbsp;", htmlspecialchars($playerInfo["displayname"]));
							$content .="</td></tr></table>\n";
