...
//      $this->debug("Test old:${ltag} <br>");
      ...
      //////////////////////////////////////////////////
//      if(ereg(REGEXP_NOENDTAG, $ltag)) {
      if(preg_match(REGEXP_NOENDTAG, $ltag)) {  // see also line 63 for delimiter attampt...
        // Previous tag does not have CloseTag.
        // Restore CSS
        $this->debug("Forced revert of ${ltag}<br>");
        $this->RevertCSSproperties($ltag);
    ...
    */
    ...
    $this->debug("LINK:$CSSlink ".$attr['HREF']."<br>");
      ...
      if(isset($attr['REL']) and isset($attr['HREF']) and (strtoupper($attr['REL'])=='STYLESHEET')) {
        if(!isset($attr['MEDIA']) or preg_match('#print#',$attr['MEDIA'])) {
           ////////////////
           //Get the file 
  ...
  //! @desc Disable some tags using ',' as separator. Enable all tags calling this function without parameters.
    if ($str == '') //enable all tags
    {
      ...
      $this->enabledtags = "<body><tt><kbd><samp><option><outline><span><newpage><page_break><s><strike><del><bdo><big><small><address><ins><cite><font><center><sup><sub><input><select><option><textarea><title><form><ol><ul><li><h1><h2><h3><h4><h5><h6><pre><strong><u><i><a><img><p><br><strong><em><code><th><tr><blockquote><hr><td><tr><table><div>";
    }
    else
    {
      $str = explode(",",$str);
