...
if (stristr($_SERVER['SCRIPT_NAME'], "block-GT_Arcade_Center.php")) {
...
if ($top) {
		...
		while ($row = $db->sql_fetchrow($result)) {
				...
				$content .= "<a href=\"forums-arcade-stat-user-".$row['user_id'].".html\"><img src=\"modules/Forums/templates/subSilver/images/loupe.gif\" border= \"0\" alt=\"Jump to $lastUser's stats...\"></a> \n";
				$content .= "<a href=\"forum-userprofile-".$row['user_id'].".html\">".$row['username']."</a> \n";
		...
		//Random Game
		$sql = "SELECT g.* , u.username FROM ".$prefix."_bbgames g, ".$prefix."_users u WHERE g.game_highuser = u.user_id ORDER BY rand() LIMIT 1";
                $row = $db->sql_fetchrow($db->sql_query($sql));
                $randomUser = $row['username'];
                $row['game_highscore'] = number_format($row['game_highscore']);

		$content .= "<td align=\"center\" class=\"row2\"><strong>".$row['game_name']."</strong><br><a href=\"forums-arcade-game-".$row['game_id'].".html\"><img src=\"modules/Forums/games/pics/".$row['game_pic']."\" border= \"0\" alt=\"".$row['game_name']."\"></a><br> High Score set by<br><strong><a href=\"forums-arcade-stat-user-".$row['game_highuser'].".html\"><img src=\"modules/Forums/templates/subSilver/images/loupe.gif\" border= \"0\" alt=\"Jump to ".$row['username']."'s stats...\"></a> <a href=\"forum-userprofile-".$row['game_highuser'].".html\">$randomUser</a> </strong><br>with <strong>".$row['game_highscore']."</strong> <br><br>Total Games <strong>$total_games</strong><br>in <strong>$total_cats</strong> categories.</td>\n";
		...
		while ($row = $db->sql_fetchrow($result)) {
				...
				$content .= "<a href=\"forums-arcade-game-$lastgameid.html\"><img src=\"modules/Forums/games/pics/$lastgamepic\" border= \"0\" alt=\"$lastGame\"><br><strong>$lastGame</strong></a><br><br>\n";
		}
...
if ($last_five) {
                $content .= "<table width=\"100%\" border=\"0\" cellpadding=\"0\" cellspaceing=\"0\">\n";
		         ...
		         $sql = "SELECT g.* , u.username FROM ".$prefix."_bbgames g, ".$prefix."_users u WHERE g.game_highuser = u.user_id ORDER BY game_highdate DESC LIMIT 0,$recent_scores";
                         $result = $db->sql_query($sql);
				...
				$content .= "<tr>\n";
				$content .= "<td width=\"18%\" align=\"center\" class=\"row1\"><a href=\"forums-arcade-stat-user-".$row['game_highuser'].".html\"><img src=\"modules/Forums/templates/subSilver/images/loupe.gif\" border= \"0\" alt=\"Jump to ".$row['username']."'s stats...\"></a><a href=\"forum-userprofile-".$row['game_highuser'].".html\">$lastUser</a> </strong></td>\n";
				$content .= "<td width=\"18%\" align=\"center\" class=\"row2\"><strong>".$row['game_highscore']."</strong></td>\n";
				$content .= "<td width=\"34%\" align=\"left\" class=\"row1\"><strong>&nbsp;&nbsp;<a href=\"forums-arcade-game-".$row['game_id'].".html\"><img height=\"20\" width=\"20\" src=\"modules/Forums/games/pics/".$row['game_pic']."\" border= \"0\" alt=\"".$row['game_name']."\"></a> - ".$row['game_name']."</strong></td>\n";
...
if ($arcade_stats) {
		...
		$content .= "<tr>\n";
                $content .= "<td width=\"50%\" align=\"center\" class=\"row1\">Most Popular: <a href=\"forums-arcade-game-".$row['game_id'].".html\"><strong>".$row['game_name']."</strong> </a>with <strong>".$row['game_set']."</strong> plays.</td>\n";

                //Least Played Game
		$sql = "SELECT game_set, game_name, game_id FROM ".$prefix."_bbgames ORDER BY game_set ASC LIMIT 1";
                $row = $db->sql_fetchrow($db->sql_query($sql));

		$content .= "<td width=\"50%\" align=\"center\" class=\"row1\">Least Popular: <a href=\"forums-arcade-game-".$row['game_id'].".html\"> <strong>".$row['game_name']."</strong> </a>with <strong>".$row['game_set']."</strong> plays.</td>\n";
		$content .= "</tr>\n";
		...
		while ($row = $db->sql_fetchrow($result)) 
                {
                	$content .="<a href=\"forums-arcade-cat-".$row['arcade_catid'].".html\">\n";
			$content .="<strong>".$row['arcade_cattitle']." (".$row['arcade_nbelmt']." Games)</B>\n";
			$content .="</a>&nbsp;&nbsp;&nbsp;\n";
		}
                $content .= "</marquee></td>\n";
                $content .= "</tr>\n";
...
if ($whos_playing) {
		...
		$sql = "SELECT u.username, u.user_id, u.user_level, user_allow_viewonline, g.game_name, g.game_id FROM ".$prefix."_bbgamehash gh LEFT JOIN ".$prefix."_bbsessions s ON gh.user_id = s.session_user_id LEFT JOIN ".$prefix."_users u ON gh.user_id = u.user_id LEFT JOIN ".$prefix."_bbgames g ON gh.game_id = g.game_id WHERE gh.hash_date >= s.session_time AND (" . time() . "- gh.hash_date <= 300) ORDER BY gh.hash_date DESC";
		...
		if ($nbplayers != 0) {
				...
				for ($i=0 ; $i<$nbplayers ; $i++) {
						if (!isset($listeid[ $players[$i]['user_id'] ])) {
								...
								if ($players[$i]['user_allow_viewonline']) 
                                                                {
										$player_link = '<a href=forum-userprofile-'.$players[$i]['user_id']. '.html>' . $players[$i]['username'] . '</a>';
                                                                }
                                                                else 
                                                                {
                                                                                $player_link = '<a href=forum-userprofile-'.$players[$i]['user_id']. '.html><i>' . $players[$i]['username'] . '</i></a>';
                                                                }
                ...
                //@RJR-Pwmg@Rncvkpwo@-@Eqratkijv@(e)@VgejIHZ.eqo
						if ($games_players[$key]!='') {
								$content .= "<tr>\n";
                                                                $content .= "<td width=\"30%\" class=\"row1\"><a href=\"forums-arcade-game-$key.html\">$val</a></td>\n";
